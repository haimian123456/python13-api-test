...
class BidLoan_Test(TestCase):
    ...
    def test_add(self, case):  # 测试 注册类 方法
        ...
        if '正常投资' in case.description:
            user_id=self.mysql.mysql_select_one(self.memberid_sql)[0][0]
        ...
        try:
            ...
            result = 'pass'
            print('加标成功')
            if json.loads(test_case.text)['msg'] =='加标成功':
                loan_id=self.mysql.mysql_select_one(case.sql)[0][0]
                setattr(Context,'loan_id',str(loan_id))
                # print('loan_id',self.context.loan_id)
